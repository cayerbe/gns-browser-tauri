...
export function bytesToBase64(bytes: Uint8Array) {
    // FIX: Cast buffer to ArrayBuffer to satisfy type checker
 ...
 */
export async function encryptForRecipient(plaintext: string, recipientX25519Hex: string) {
 ...
 */
export async function tryDecryptMessage(msg: any) {
    // Ensure sodium is initialized
    if (!sodium) {
        await initCrypto();
    }
 ...
 */
export async function sign(data: string, privateKeyHex: string) {
 ...
 * Sign a GNS envelope
 */
export async function signEnvelope(envelope: any, privateKeyHex: string) {
 ...
 */
export async function signMessage(toPk: string, payload: any, privateKeyHex: string) {
 ...
 */
export async function verify(data: string, signatureHex: string, publicKeyHex: string) {
    try {
